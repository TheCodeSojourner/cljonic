...
int main()
{
    ...
    const auto t0{Drop(0, a)};                               // immutable, full Array, with 0 to 9
    const auto t1{Drop(1, a)};                               // immutable, sparse Array, with 1 to 9
    const auto t5{Drop(5, a)};                               // immutable, sparse Array, with 5 to 9
    const auto t50{Drop(50, a)};                             // immutable, empty Array
    const auto tEmpty{Drop(50, Range<0>{})};                 // immutable, empty Array
    const auto tRpt7{Drop(5, Repeat<7, const char*>{"11"})}; // immutable, sparse Array, with two "11"s
    const auto tSet5{Drop(5, Set{'a', 'b'})};                // immutable, empty Array
    const auto tStr3{Drop(3, String{"Hello"})};              // immutable, sparse Array, with 'l', and 'o'
