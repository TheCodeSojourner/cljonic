...
cat cljonic-collection-type.hpp \
    ...
    # cljonic-core-takewhile.hpp \

# remove unneeded lines
sed -i '/^#include "/d' /tmp/cljonic-glued.hpp
sed -i '/^#ifndef CLJONIC_/d' /tmp/cljonic-glued.hpp
sed -i '/^#define CLJONIC_/d' /tmp/cljonic-glued.hpp
sed -i '/^#endif\s\+\/\/\s\+/d' /tmp/cljonic-glued.hpp

# remove all the comments
g++ -std=c++23 -fpreprocessed -dD -E -o /tmp/cljonic.hpp /tmp/cljonic-glued.hpp
